...
//
//
// カプセル( 移動している球体 ) と AABB の衝突判定と補正サンプル
//
//
//-----------------------------------------------------------------------------------------------------------
...
tnl::Vector3 box_pos = { 100, 0, 50 };
tnl::Vector3 box_size = { 80, 70, 60 };
tnl::Quaternion box_rot;
...
void gameStart() {
	...
	box_rot = tnl::Quaternion::RotationRollPitchYawFromVector({ tnl::ToRadian(20), tnl::ToRadian(50), tnl::ToRadian(45) });
}
...
void gameMain(float delta_time) {
	...
	float t = 0;
	if (tnl::IsIntersectCapsuleAABB(cap_st, cap_en, cap_r, tnl::ToMaxAABB(box_pos, box_size), tnl::ToMinAABB(box_pos, box_size), &t)) {
	...
	dxe::DrawAABB(camera, box_pos, box_size, dxe::Colors::DeepPink);
