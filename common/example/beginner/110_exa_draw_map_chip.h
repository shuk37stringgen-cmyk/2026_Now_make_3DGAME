...
// サンプルデータではスプライトシートに８種類のチップが存在
const int CHIP_TYPE_NUM = 8;

// スプライトシートから分割ロードしたグラフィックハンドルの格納用配列
int chip_graphics_handles[CHIP_TYPE_NUM];
...
void gameStart() {
	...
	LoadDivGraph("resource/graphics/example/test_map_chip.bmp", CHIP_TYPE_NUM, 4, 2, CHIP_SIZE, CHIP_SIZE, chip_graphics_handles);
...
void gameEnd() {
	...
	// LoadDivGraph によって確保された画像のメモリを解放
	for (int i = 0; i < CHIP_TYPE_NUM; ++i) {
		DeleteGraph(chip_graphics_handles[i]);
	}
}
