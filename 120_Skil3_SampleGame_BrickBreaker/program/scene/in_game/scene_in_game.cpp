...
void SceneInGame::lazyInitialize() {
	...
	// バーオブジェクトの生成と登録
	Shared<Bar> bar = std::make_shared<Bar>();
	addObject(bar);
	p_bar_ = bar;
	...
	// ボールオブジェクトの生成と登録
	Shared<Ball> ball = std::make_shared<Ball>();
	addObject(ball);
	p_ball_ = ball;
	...
	float sx = 290;
	float sy = 100;
	for (int i = 0; i < Block::BLOCK_NUM_H; ++i) {
		for (int k = 0; k < Block::BLOCK_NUM_W; ++k) {
			Shared<Block> p_block = std::make_shared<Block>();
			p_block->setPosition({ sx + k * Block::BLOCK_SIZE_W, sy + i * Block::BLOCK_SIZE_H });
			p_block->setSize({ Block::BLOCK_SIZE_W, Block::BLOCK_SIZE_H });
